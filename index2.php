...
<head>
    ...
    <script type="module">
        // =================================================================
        // KONFIGURASI FIREBASE (GANTI DENGAN KODE ANDA!)
        // =================================================================
        ...
        const greetingsRef = db.ref('greetings'); // Node utama di Realtime DB

        // =================================================================
        // LOGIKA FIREBASE: MENGIRIM UCAPAN
        // =================================================================
        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            // Panggil listener untuk menampilkan daftar ucapan
            setupGreetingListener();
        });

        function handleGreetingSubmit(e) {
            e.preventDefault();
            ...
            const successDiv = document.getElementById('greetings-success');
            const errorDiv = document.getElementById('greetings-error');
            ...
            if (!name || !message) {
                errorDiv.textContent = "Nama dan Ucapan tidak boleh kosong.";
            ...
            greetingsRef.push({
                name: name,
                message: message,
                timestamp: firebase.database.ServerValue.TIMESTAMP // Waktu server
            })
            .then(() => {
            ...
            .catch((error) => {
                errorDiv.textContent = "Gagal menyimpan ucapan: " + error.message;
                errorDiv.style.display = 'block';
        ...
        // =================================================================
        // LOGIKA FIREBASE: MENDENGARKAN DAN MENAMPILKAN UCAPAN
        // =================================================================

        function setupGreetingListener() {
            const greetingsList = document.getElementById('greetings-list');

            // Ambil 50 ucapan terbaru, diurutkan berdasarkan timestamp
            greetingsRef.limitToLast(50).orderByChild('timestamp').on('value', (snapshot) => {
                greetingsList.innerHTML = ''; // Kosongkan daftar

                if (snapshot.exists()) {
                    const greetings = [];
                    // Iterasi snapshot dan masukkan ke array
                    snapshot.forEach((childSnapshot) => {
                        greetings.push(childSnapshot.val());
                    });
                    
                    // Balikkan array agar yang terbaru muncul di atas
                    greetings.reverse();
                    ...
                    greetings.forEach(greeting => {
                        ...
                        const greetingHtml = `
                            <div class="bg-white p-4 rounded-xl shadow-md mb-4 border border-primary-gold/30 animate-fadeIn text-left">
                                <p class="font-serif-elegant text-lg text-secondary-gold font-bold mb-1">${escapeHtml(greeting.name)}</p>
                                <p class="font-inter text-sm text-secondary-dark leading-relaxed mb-3">${escapeHtml(greeting.message).replace(/\n/g, '<br>')}</p>
                                <p class="text-xs text-gray-400 font-inter">${formattedDate}</p>
                            </div>
                        `;
                        greetingsList.innerHTML += greetingHtml;
                    });
                } else {
                    greetingsList.innerHTML = '<p class="text-center text-text-dark">Belum ada ucapan. Jadilah yang pertama!</p>';
                }
            }, (error) => {
                console.error("Error reading from database:", error);
                greetingsList.innerHTML = '<p class="text-center text-red-500">Gagal memuat ucapan.</p>';
            });
        }

        // Fungsi kecil untuk membersihkan HTML
    ...
    <style>
        ...
        /* MODIFIKASI: MENGHILANGKAN SHADOW/OVERLAY GELAP */
        #hero-video-section::after {
            ...
            width: 100%;
            height: 100%;
            background: transparent; /* Diubah dari rgba(0, 0, 0, 0.2) menjadi transparent */
            z-index: 15;
        }
        ...
        #hero-text-overlay {
            ...
            /* text-shadow: 1px 1px 5px rgba(0,0,0,0.5); <-- Dihilangkan agar lebih cerah */
            
            /* BARU: Sembunyikan dan beri transisi untuk efek muncul setelah 4 detik */
            opacity: 0;
            transition: opacity 1s ease-in-out;
        }
        ...
        #countdown-section {
            ...
            background-position: center;
            background-repeat: no-repeat;
            /* MODIFIKASI: Meningkatkan padding-top dari 5rem menjadi 10rem untuk menggeser font dan konten ke bawah */
            padding: 10rem 1rem; 
            width : 100%;
            height: 100vh; 
        }
...
<section id="hero-video-section">
            <video id="hero-video" autoplay muted playsinline>
                <source src="https://undanganfaniadelia.github.io/Custom-Maylia-Ibam-Rev-Fix-brake.mp4" type="video/mp4">
...
<div id="hero-text-overlay">
        ...
        <div class="ayat-frame rounded-xl text-center flex flex-col items-center justify-center">
            ...
            <p lang="ar" dir="rtl" class="font-serif text-gold-dark quran-ayat mt-4">
                وَمِنْ اٰيٰتِهٖٓ اَنْ خَلَقَ لَكُمْ مِّنْ اَنْفُسِكُمْ اَزْوَاجًا لِّتَسْكُنُوْٓا اِلَيْهَا وَجَعَلَ بَيْنَكُمْ مَّوَدَّةً وَّرَحْمَةً ۗاِنَّ فِيْ ذٰلِكَ لَاٰيٰتٍ لِّلْقَوْمٍ يَّتَفَكَّرُوْنَ
            </p>
...
<form id="greeting-form" class="space-y-4 text-left">
                    <div id="greetings-success" class="p-3 mb-4 text-sm text-green-700 bg-green-100 rounded-lg hidden" role="alert">
                        Ucapan dan doa Anda berhasil terkirim!
                    </div>
                    <div id="greetings-error" class="p-3 mb-4 text-sm text-red-700 bg-red-100 rounded-lg hidden" role="alert">
                        </div>
                    
                    
                    <div class="font-semibold text-sm text-secondary-dark">Nama Anda</div>
                    <input type="text" id="guestName" placeholder="Contoh: Bpk. Fulan" class="w-full p-3 border border-gray-300 rounded-lg focus:outline-none focus:border-primary-gold" required>
                    
                    <div class="font-semibold text-sm text-secondary-dark">Ucapan & Doa</div>
                    <textarea id="guestMessage" placeholder="Contoh: Selamat menempuh hidup baru, semoga sakinah mawaddah warahmah!" rows="4" class="w-full p-3 border border-gray-300 rounded-lg focus:outline-none focus:focus:border-primary-gold" required></textarea>
...
<div id="greetings-list" class="mt-4 p-2">
                    ...
                    <p class="text-center text-text-dark">Memuat ucapan...</p>
...
<div class="bank-card">
    ...
    <script>
        ...
        document.getElementById('open-btn').addEventListener('click', function() {
            ...
            const heroTextOverlay = document.getElementById('hero-text-overlay'); // BARU: Ambil elemen teks overlay
            ...
            // Pastikan video dimulai saat cover dibuka
            if (heroVideo) {
                ...
                // Hapus loop video: memastikan video berhenti saat durasi habis
                heroVideo.onended = () => {
                    heroVideo.pause(); 
                };

                // MODIFIKASI: Tampilkan teks overlay setelah 9 detik
                if (heroTextOverlay) {
                    setTimeout(() => {
